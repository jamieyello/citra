...
static void AllocateBuffer(Service::Interface* self) {
    ...
    if (shared_memory) {
        ...
        ResultCode r = shared_memory->Map(Kernel::g_current_process.get(), 0,
            shared_memory->permissions, shared_memory->other_permissions);
        LOG_WARNING(Service_MIC, "(STUBBED) called, result=0x%08X", r);
    }
