...
struct DnsServerInfo {
    DnsAddr entry[4] = {
        {AF_INET, {8, 8, 8, 8}}, {AF_INET, {8, 8, 4, 4}},
    };
    char domain[256] = {
        0,
    };
...
struct {
    u32 ip = 0x20202020;
    u32 netmask = 0xFFFF0000;
    u32 subnet_broadcast = 0xFFFF0000;
} ip_addr_table;
...
static void GetNetworkOpt(Service::Interface* self) {
    ...
    switch (no) {
    case NetworkOpt::NETOPT_IP_ADDR_TABLE: {
        Memory::WriteBlock(optval, &ip_addr_table, sizeof(ip_addr_table));
        cmd_buff[2] = 0;
        cmd_buff[3] = 0xC;
    } break;
    ...
    cmd_buff[1] = RESULT_SUCCESS.raw;
    LOG_WARNING(Service_SOC, "(STUBBED) called, level=0x%X, optname=0x%X, addr=0x%08X, size=0x%X",
                level, optname, optval, size);
}
